...
(function ($, win, doc, body) {
    ...
    function countSelectedItems($itemList) {
        ...
        if (selectedCount > 0) {
            $countBox.text(selectedCount + " " + "selected").show();
        } else {
            $countBox.text("").hide();
        }
    }
    ...
    advancedSelector.prototype = {
        ...
        init: function () {
            ...
            var $o = $(that.options.templates.selectorContainer).tmpl({ opts: that.options });
            if (that.options) {
        ...
        displayAddItemBlock: function (data) {
            ...
            var $o = $(that.options.templates.addNewPanel).tmpl({ fields: data.newoptions, btnTitle: data.newbtn });
        ...
        actionsAfterCreateItem: function (data) {
            ...
            var $o = $(that.options.templates.itemList).tmpl( { Items: [newObj], isJustList: that.options.onechosen });
        ...
        initDataSimpleSelector: function (data) {
            var that = this,
                objType = data.tag,
                list = $(that.options.templates.addNewItems).tmpl({ Items: data.items }),
        ...
        showItemsListAdvSelector: function () {
            ...
            if (that.options.showme) {
                ...
                for (var i = 0, length = itemsDisplay.length; i < length; i++) {
                    if (itemsDisplay[i].id == TeamlabProfileId) {
                        user = itemsDisplay[i];
                        user.title = "Me";
            ...
            var $items = $(that.options.templates.itemList).tmpl( { Items: itemsDisplay, isJustList: that.options.onechosen }),
                height;
        ...
        showGroupsListAdvSelector: function () {
            var that = this,
                $groups = $(that.options.templates.groupList).tmpl( { Items: that.groups, isJustList: that.options.onechosen });
